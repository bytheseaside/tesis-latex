...
\begin{table}[h]
\centering
\caption[Forward matrix properties comparison]{Comparison of monopolar and dipolar forward matrix properties.}
...
\begin{table}[h]
\centering
\caption[Probe discrimination statistics]{Probe discrimination statistics across all 21 measurement locations. CV (coefficient of variation) quantifies how much probe responses vary across different source configurations; higher CV indicates better spatial discrimination capability.}
...
% Mean probe std (mV) & 29.3 & 34.4 \\ no es muy relevante, dado que el CV muestra la misma data y más
\bottomrule
\end{tabular}
\end{table}
...
Figures~\ref{fig:mono-cases}--\ref{fig:dip-heatmap} show the electric potential
distribution for all stimulation cases. Visual inspection
...
\caption[Monopolar field distributions]{Electric potential distributions for the 9 monopolar stimulation cases.
...
\caption[Monopolar response heatmap]{Heatmap of probe voltage responses for monopolar stimulation. Each row
represents one of the 21 probes, each column represents one of the 9 monopolar electrode
...
\caption[Dipolar field distributions]{Electric potential distributions for the 36 dipolar stimulation cases.
...
\caption[Dipolar response heatmap]{Heatmap of probe voltage responses for dipolar stimulation. Each row
...
\begin{enumerate}
  ...
  \item \textbf{Superior spatial discrimination}: Probe responses show $\approx15\times$ higher
...
\subsection{Duda: Análisis de Correlación entre protocolos}

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{../../02_Images/mono_correlation_heatmap.pdf}
\caption[Monopolar correlation matrix]{Correlation in monopolar protocol. RMS: 0.4056}
\label{fig:mono-corr}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{../../02_Images/dip_correlation_heatmap.pdf}
\caption[Dipolar correlation matrix]{Correlation in dipolar protocol. RMS: 0.4553}
\label{fig:dip-corr}
\end{figure}

\nota{Nota para revisión:} 
El RMS de la correlación entre casos es similar e incluso un poco más alta en el dipolar
--- corr(D) = 0.4553 (Fig~\ref{fig:dip-corr}) vs corr(M) = 0.4056 (Fig~\ref{fig:mono-corr}) --- a pesar de que todas las otras métricas favorecen al dipolar.
De todas formas, del conjunto dipolar podemos eliminar columnas (muchas más que en monopolar) mediante algoritmos de
selección, lo que debería reducir este número. La magnitud de este RMS representa la redundancia inherente en los 36
casos dipolares, algo similar a lo mencionado antes sobre el número de condición, que también refleja redundancia/dependencia lineal. 

Deberíamos incluir esta métrica de RMS de correlación en la comparación principal y justificar por qué un valor ligeramente (0.05) mayor
en dipolar no contradice nuestra decisión? O podemos omitirla y justificar la decisión basandonos en las otras métricas?
