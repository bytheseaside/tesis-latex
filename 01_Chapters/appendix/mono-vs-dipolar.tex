...
voltages measured at the 21 probes (10--20 EEG system) for one unit source activation.
...
Rank & 9 & 21 \\
Condition number ($\kappa$) & 128 & 10{,}424 \\
Max singular value & 0.937 & 0.649 \\
Min singular value & $7.3 \times 10^{-3}$ & $6.2 \times 10^{-5}$ \\
Singular value decay & 2 orders of magnitude & 4 orders of magnitude \\
\bottomrule
\end{tabular}
\end{table}
...
spanning the entire probe measurement space. Its poor condition number ($\kappa \approx 10{,}424$)
...
\paragraph{Example: Probe Fp2 (frontal-right).}
This probe exhibits maximum contrast between protocols:
\begin{itemize}
  \item \textbf{Monopolar}: 64.6 $\pm$ 16.7~mV (CV = 26\%). Regardless of which 
        electrode was activated, Fp2 consistently measured $\sim$65~mV.
  
  \item \textbf{Dipolar}: $-1.5$ $\pm$ 25.1~mV (CV = 1627\%). Different dipoles 
        produced dramatically different responses spanning both positive and negative values.
\end{itemize}

The 63-fold increase in CV for this probe (and 15-fold average across all probes)
demonstrates that dipolar stimulation provides dramatically superior spatial discriminability.
