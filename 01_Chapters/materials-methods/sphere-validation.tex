After deriving the governing equation and defining the discretization strategy employed for the phantom--head simulations, it is necessary to quantify the numerical error introduced by the finite element approximation. Since no analytical solution exists for the full phantom geometry, this assessment must be carried out on a simplified configuration for which a closed--form reference solution is available (derived analytically under symmetry assumptions, see \textbf{Appendix}~\ref{appendix:laplace-r}).
