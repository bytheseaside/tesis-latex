...
a three-dimensional finite element mesh using Gmsh was generated~\cite{gmsh}. The mesh was
...
Each of the nine internal electrodes is modeled as a cylindrical contact (3.5~mm diameter, 12~mm tall~\cite{jack_datasheet}) that imposes
...
\paragraph*{Physical Groups and Mesh Entities}
Inside the gel volume, nine embedded electrode patches (labeled $v_1,\dots,v_9$) serve as voltage-driven 
sources. Each electrode is modeled as a cylindrical surface matching a Jack 3.5~mm audio connector 
(model SP-35501-OM), and is assigned a physical surface tag (IDs 11--19 for electrodes $v_1$ through $v_9$). 
During simulation, these surfaces are held at prescribed potentials $u = V_i(t)$ via Dirichlet boundary conditions.
The two volume regions are tagged as physical volumes: the gel interior receives physical tag~2, and the 
conductive PLA shell receives physical tag~1. These tags allow straightforward assignment of material 
properties (piecewise constant conductivity) and boundary conditions in the finite element solver. 
The outer shell surface was left untagged, which implicitly enforces a natural (Neumann, insulating) 
boundary condition at the phantom exterior.