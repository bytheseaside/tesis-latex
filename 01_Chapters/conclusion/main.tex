...
Together, these results demonstrate that the proposed phantom head, numerical characterization, and software environment form a coherent and reproducible framework for controlled EEG experimentation.

\clearpage
\section{Future Work}

The following sections detail software and physical phantom enhancements designed to evolve the current system into a closed, production-ready environment for \gls{bci} testing.

\subsection{Software Platform Enhancements}

Future iterations of the software platform should focus on optimizing and extending its capabilities. Key areas for development include:

\begin{itemize}
    \item \textbf{Scalability and robustness:} To address the memory constraints already mentioned, future iterations should implement paginated disk buffering. Additionally, error handling should be enhanced, particularly regarding audio subsystem failures. A more granular exception-handling strategy is necessary when hardware peripherals fail.

    \item \textbf{Integration of the model-based excitation and surface response control:} The numerical characterization developed in this work defines a conditioned mapping between internal actuation channels and surface-level measurements through a forward operator and its associated inverse. Future versions of the application should integrate these operators to support model-driven workflows that abstract low-level actuation details while remaining consistent with the physically realizable system response.
